...
export function Navigation({ locale, ui, navigation, profile }: NavigationProps) {
  ...
  return (
    ...
    >
      <nav className="mx-auto w-full max-w-7xl px-4 py-3 md:hidden">
        ...
        <div className="mt-3 flex items-center gap-1 overflow-x-auto rounded-full border border-white/10 bg-white/[0.035] p-1 backdrop-blur-xl">
          {navigation.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="focus-ring whitespace-nowrap rounded-full px-3 py-2 text-xs font-medium text-white/64 transition hover:bg-white/8 hover:text-white"
            >
              {item.label}
            </a>
          ))}
      ...
      <nav className="mx-auto hidden min-h-16 w-full max-w-7xl items-center justify-between gap-4 px-4 md:flex md:px-8">
        <a
          href="#hero"
          className="focus-ring font-mono text-xs uppercase tracking-[0.22em] text-white/78 transition hover:text-white"
        >
